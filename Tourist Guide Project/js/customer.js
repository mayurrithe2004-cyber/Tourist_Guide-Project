...
if(!state){
    alert('No demo data found. Open index.html first.');
...
function renderAuthArea(){
    ...
    if(!currentUser){
        ...
        <div style="display:flex;justify-content:space-between;align-items:center">
          <div><strong>Welcome — please login</strong><div class="muted">Demo: user@demo.com / pass123</div></div>
          <div><button class="btn" onclick="openLogin()">Login</button>
          <button class="btn" onclick="openRegister()">Register</button></div>
        </div>`;
    } else {
        el.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center">
...
function renderDestList(){
    const grid = document.getElementById('destList');
    grid.innerHTML='';
    state.destinations.forEach(d=>{
        ...
        card.innerHTML=`
          ...
          <h4>${d.name} <span style="float:right;font-weight:700">₹${d.price}</span></h4>
          <p class="muted">${d.location}</p>
          <div style="margin-top:8px">
            <button class="btn" onclick="book(${d.id})">Book</button>
            <button style="margin-left:8px" onclick="toWishlist(${d.id})">Wishlist</button>
          </div>`;
...
function renderBookings(){
    ...
    if(bookings.length===0) el.innerHTML='<div class="muted">No bookings</div>';
        ...
        row.innerHTML=`<strong>${d.name}</strong><div class="muted">${b.travelDate} • ₹${b.amount}</div>
        <div style="margin-top:6px"><button class="btn" onclick="cancel(${b.id})">Cancel</button></div>`;
        el.appendChild(row);
    });
}
...
function renderProfile(){
    ...
    el.innerHTML = `<p><strong>${currentUser.name}</strong></p>
                    <p class="muted">${currentUser.email}</p>
                    <p class="muted">Bookings: ${currentUser.bookings.length}</p>`;
}
...
function renderWishlist(){
    ...
    if(list.length===0) el.innerHTML='<div class="muted">Wishlist empty</div>';
        ...
        w.innerHTML=`<strong>${d.name}</strong><div class="muted">${d.location}</div>
        <div style="margin-top:6px">
        <button class="btn" onclick="book(${d.id})">Book</button>
        <button style="margin-left:8px" onclick="removeWish(${d.id})">Remove</button>
        </div>`;
        el.appendChild(w);
    });
}
...
function openLogin(){
    ...
    if(email===state.admin.email && pass===state.admin.pass){ alert('Admin login'); location.href='admin.html'; return; }
...
function openRegister(){
    ...
    const u = {id,name,email,pass,bookings:[],wishlist:[]};
    state.users.push(u);
...
function logout(){
    sessionStorage.removeItem('tg_user'); currentUser=null; renderAll();
}

function book(destId){
    if(!currentUser){ alert('Login to book'); openLogin(); return; }
    ...
    const b = {id,touristId:currentUser.id,destinationId:dest.id,packageName:dest.name,
        bookingDate:new Date().toISOString().slice(0,10),
        travelDate:date,amount:dest.price*qty,qty};
...
function cancel(id){
    ...
    state.users.forEach(u=>u.bookings=u.bookings.filter(x=>x!==id));
    localStorage.setItem(LS,JSON.stringify(state));
    sessionStorage.setItem('tg_user',JSON.stringify(state.users.find(x=>x.id===currentUser.id)));
    currentUser=JSON.parse(sessionStorage.getItem('tg_user'));
    renderAll();
}

function toWishlist(id){
    if(!currentUser){ alert('Login to use wishlist'); openLogin(); return; }
...
/* -------------------- Quick Book auto -------------------- */
const quickId = sessionStorage.getItem('tg_quick');
if(quickId){
    sessionStorage.removeItem('tg_quick');
